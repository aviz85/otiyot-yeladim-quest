...
const PRINT_pressButton = async (color)=>{
  ...
  if(color===order[f.press_step]){
    ...
    await G.say('noam','המכונה עשתה ״אפְּצ׳י!״ וירקה עליי דף ריק. מתחילים מההתחלה: ירוק, צהוב, אדום.');
  }
};
...
registerRoom({
  ...
  hotspots:[
    ...
    {name:'חביות דיו', x:202,y:112,w:22,h:34, wx:212,
      ...
      use: async()=>{
        await G.say('noam','אני לא נוגע. כתם דיו לא יורד גם אחרי שלושה מקלחות ושתי סבתות עם מגבת.');
      }},
      ...
      use: async()=>{
        ...
        if(!G.has('flashlight') && !f.flash_ok && !f.got3){
          ...
          if(c===0){
            ...
            await G.say('shimon','(בשקט) באמת? וואו. שקט. מוזר לי. (חוזר לצעוק) בכל מקרה!! ניירות של רבקה ירדו לארכיון!! במרתף!! חושך שם מצרים!!');
          }
        ...
        if(G.has('flashlight') && !f.flash_ok){
          ...
          await G.say('shimon','מה?? מצאת סוללות?? עוד לא?? תחפש בשלטים!! סוללות אוהבות שלטים שמקרינים רק תמונה אחת של חתול!!');
        ...
        if(n===3){
          ...
          await G.say('shimon',`${n} מתוך שלושה?! יופי!! המכונה מחכה!! היא לא סבלנית אבל היא מחכה!!`);
